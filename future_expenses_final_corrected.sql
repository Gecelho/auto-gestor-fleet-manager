-- SQL FINAL CORRIGIDO para criar funcionalidade de despesas futuras
...
-- 2. Criar tabela para armazenar quilometragem atual dos carros
CREATE TABLE IF NOT EXISTS car_current_mileage (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  car_id UUID NOT NULL REFERENCES cars(id) ON DELETE CASCADE,
  current_mileage NUMERIC(10,0) NOT NULL DEFAULT 0,
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
  UNIQUE(car_id)
);

-- 3. Criar índice para performance
CREATE INDEX IF NOT EXISTS idx_car_current_mileage_car_id ON car_current_mileage (car_id);

-- 4. Inserir dados iniciais na tabela de quilometragem atual (usando a quilometragem da compra como base inicial)
INSERT INTO car_current_mileage (car_id, current_mileage)
SELECT id, COALESCE(mileage, 0)
FROM cars
ON CONFLICT (car_id) DO NOTHING;

-- 5. Criar tabela para controlar despesas futuras baseadas em quilometragem
...
DROP TRIGGER IF EXISTS update_car_current_mileage_updated_at ON car_current_mileage;
CREATE TRIGGER update_car_current_mileage_updated_at 
    BEFORE UPDATE ON car_current_mileage 
    FOR EACH ROW 
    EXECUTE FUNCTION update_updated_at_column();

-- 9. Criar view CORRIGIDA para facilitar consultas de despesas futuras
...
SELECT 
    ...
    e.next_mileage,
    e.paid,
    ccm.current_mileage as current_car_mileage,
    c.name as car_model,
    c.name as car_brand,
    c.plate as car_plate,
    (fe.target_mileage - ccm.current_mileage) as km_remaining,
    CASE 
        WHEN (fe.target_mileage - ccm.current_mileage) <= 1000 AND (fe.target_mileage - ccm.current_mileage) > 0 THEN true
        ELSE false
    END as is_near
FROM future_expenses fe
JOIN expenses e ON fe.expense_id = e.id
JOIN cars c ON fe.car_id = c.id
LEFT JOIN car_current_mileage ccm ON fe.car_id = ccm.car_id
ORDER BY 
    fe.is_completed ASC,
    (fe.target_mileage - COALESCE(ccm.current_mileage, 0)) ASC;

-- 10. Função para criar automaticamente future_expenses quando uma despesa tem next_mileage
...
-- 11. Criar triggers para criar future_expenses automaticamente
...
-- 12. Função RPC CORRIGIDA para buscar próxima despesa de cada carro
...
-- 13. Migrar dados existentes (despesas que já têm next_mileage)
INSERT INTO future_expenses (car_id, expense_id, target_mileage)
SELECT 
    e.car_id,
    e.id,
...
-- 14. Função para atualizar quilometragem atual do carro
CREATE OR REPLACE FUNCTION update_car_current_mileage(p_car_id UUID, p_mileage NUMERIC)
RETURNS VOID AS $$
BEGIN
    INSERT INTO car_current_mileage (car_id, current_mileage)
    VALUES (p_car_id, p_mileage)
    ON CONFLICT (car_id) DO UPDATE SET
        current_mileage = p_mileage,
        updated_at = timezone('utc'::text, now());
END;
$$ LANGUAGE plpgsql;

-- Comentários:
-- - car_current_mileage: nova tabela para armazenar quilometragem atual dos carros
...
-- - update_car_current_mileage(): função para atualizar quilometragem atual